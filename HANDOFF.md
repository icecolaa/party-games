...
4. **平台休眠问题（未解决，平台侧）**：部署后应用「running」约 1 分钟即进入 `sleeping`（缩容），休眠期所有请求返回 **204 空响应且不自动唤醒**（带浏览器 UA 也一样）；重新部署可短暂唤醒约 1 分钟。已实测应用进程本身稳定（本地解压部署包压测 90 秒无崩溃）。→ 需用户到 PocketBay 控制台查看是否有「休眠/Keep Awake」开关，或联系平台。
5. **平台托管壳路由缺陷（已绕过，平台侧）**：访客访问任意地址时平台会套一层壳页，应用被放进 `<iframe src="party-games--e.pocketbay.app">`。该边缘对**浏览器发起的子路径文档导航**（Sec-Fetch 头请求）会间歇性改写回大厅页/返回空响应，导致「点击卡片没反应」；无 Sec-Fetch 头的 curl 拿到的始终是真实页面（此前线上验证全绿的坑就在这）。已在大厅页用 **fetch 装载导航**绕过（`public/index.html` 内联脚本，`tests/lobby-nav-test.js` 8 项回归锁定）：点卡片改 fetch+document.write，深链接/刷新/邀请链接被改写时自动恢复真实页面，popstate 统一 reload。注意 `document.open()` 会清除 window 上全部监听器，勿依赖跨 write 的监听存活。
6. 部署包自检要点：仅 1 个 package.json（根）、`apps/` 下无 server.js、含 `public/` 与 `servers/`。

## 五、遗留待办

1. **平台休眠问题（唯一硬遗留，平台侧，需用户操作）**：应用部署后 running 约 1 分钟即 `sleeping`，休眠期所有请求返回 204 空响应且不自动唤醒，休眠期访客看到空白页。→ 待用户到 PocketBay 控制台确认是否有「休眠/Keep Awake」开关，或联系平台。应用本身无问题（2026-09-11 已重新部署并在线上验证通过）。
2. **托管壳路由缺陷**：已绕过（见 §四.5），但建议向平台报告「子路径文档导航被改写」与「休眠不唤醒」两个问题。
