...
1. **部署**：等用户在控制台确认休眠设置（或平台恢复唤醒）后，`npm run deploy` 重新部署并线上验证 9 路由 + 联机建房 + 规则弹窗。
2. **验收报告 v2 回填**：`E:\code\party-games-acceptance\验收报告-v2.md` 的「§6 部署验收」章节待线上验证后补结论。
3. 小事：本机 8600 端口可能仍有旧 node 服务在跑（`netstat -ano | grep 8600`），新会话启动前可先停。
